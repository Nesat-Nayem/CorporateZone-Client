...
const Applicant = () =>
{
    ...
    useEffect(() =>
    {
        const matchFound = postedJobs.filter(posted => posted.companyEmail === currentUser.email);
    ...
    return (
        <>
            <h2 className="text-center p-1 text-lg bg-slate-500 rounded-2xl font-semibold text-white">
                You've posted <span className="bg-white text-black rounded-2xl p-1">{matchJobs.length}</span> job so far
            </h2>
            {
                matchJobs.map(job => <OpenJobs
                    key={job._id}
                    id={job._id}
                    title={job.jobTitle}
